...
if(isset($_SESSION['login_name'])){
	...
	if (USER_IS_ADMIN != 'yes' && USER_IS_PUBLISHER != 'yes') {
        exit;
    }
        ...
        case 'get_news_by_title': {
            ...
            if (strlen($term) > 1) {
                $temp = $db->selectElem(DB_T_PREFIX."news",
                    "n_id, n_title_ru, n_title_ua, n_title_en, n_sign_url",
                    "n_is_active = 'yes' AND n_title_ru LIKE '%".$term."%' ORDER BY n_datetime_add DESC LIMIT 15");
                if ($temp) {
                    foreach ($temp as $item){
                        $title = $item['n_title_ru'] ? $item['n_title_ru'] : ($item['n_title_ua'] ? $item['n_title_ua'] : $item['n_title_en']);
                        $news[] = array(
                            'id' => $item['n_id'],
                            'value' => trim(strip_tags($title)),
                            'address' => $item['n_sign_url']
                        );
                    }
                }
            }
