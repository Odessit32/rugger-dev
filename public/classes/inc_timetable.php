...
if ($date_show){
    if (strlen($date_show)==10){
        ...
        if (!empty($date_show_a[0]) && !empty($date_show_a[1]) && !empty($date_show_a[2]) &&
            ...
            switch($date_type){
                case 'date':
                    $date_now = date("Y-m-d", strtotime($date_show)).' 00:00:00';
                    break;
                case 'week':
                    ...
                    $date_now = date("Y-m-d", strtotime($date_show.' -'.$w_day.' days')).' 00:00:00';
                    break;
                case 'month':
                    $date_now = date("Y-m-1", strtotime($date_show)).' 00:00:00';
                    break;
            }
        }
    }
}
$date_now_old = $date_now;
$smarty->assign("timetable_list", $timetable->getTimetableSoon($date_type, $date_now, $one_ch, $is_empty));
$championships_wg_list = $timetable->getChampionshipsWGroups('time');
$ch_title = '';
...
if (strtotime(date("Y-m-01 00:00:00" ,strtotime($date_now))) >  strtotime(date("Y-m-01 00:00:00" ,strtotime($date_now_old)))){
    $is_prev = false;
}
...
switch($date_type){
    ...
    case 'week':
        ...
        break;
    case 'month':
        if (strtotime(date("Y-m-t 23:59:59", strtotime($date_now.' -1 month'))) > time() && $is_prev){
...
$smarty->assign("ch_title", $ch_title);
...
//$one_year = true;
//$one_month = false;
//$smarty->assign("timetable_list", $timetable->getTimetableSoon($date, $get_ch_id, $one_year, $one_month));
