	...
	<table width="100%" cellspacing="0" cellpadding="0" border="0">
		...
		<tr>
			...
			<td width="25%" id="active_right"><a href="?show=club&get=stadium">Стадионы</a></td>
		</tr>
	</table>
	...
	<br>
		<table width="100%" cellspacing="0" cellpadding="0" border="0">
			...
			<tr>
				<td width="33%" id="{if $smarty.get.get == 'stadium'}active_left{else}notactive{/if}"><a href="?show=club&get=stadium">Список</a></td>
				<td width="34%" id="{if $smarty.get.get == 'stadiumedit'}active{else}notactive{/if}">{if $smarty.get.item>0}<a href="?show=club&get=stadiumedit&item={$smarty.get.item}">Редактировать</a>{else}Редактировать{/if}</td>
				<td width="33%" id="{if $smarty.get.get == 'stadiumadd'}active_right{else}notactive{/if}"><a href="?show=club&get=stadiumadd">Добавить</a></td>
			</tr>
		</table>
	...
	{if $smarty.get.get == 'stadium'}
		...
		<center>
		{if $club_stadium_list and $club_country_list}
			<table width="80%" cellspacing="0" cellpadding="0" border="0" id="form_input">
			{foreach item=item_country from=$club_country_list name=club_country}
				...
				{foreach key=key item=item from=$club_stadium_list name=club_stadium}
				{if $item_country.cn_id == $item.std_cn_id}
					<tr onmouseover="javascript: this.style.background = '#eee'" onmouseout="javascript: this.style.background = ''">
						<td><a href="?show=club&get=stadiumedit&item={$item.std_id}">{if $item.std_title_ru == ''}<font style="color: #f00;">Без названия</font>{else}{$item.std_title_ru}{/if}</a></td>
					...
					<td width="50%">
						...
						<tr>
							...
							<td width="50%" align="center">
								...
								{foreach key=key item=item from=$club_country_list name=categories}{if $item.cn_id>0}
						...
						<tr>
							...
							<td width="50%" align="center" id="city">
								...
								{foreach key=key item=item from=$club_city_list name=categories}{if $item.ct_cn_id>0}
						...
						<tr>
							...
							<td width="50%" align="center" id="city">
								...
								{foreach key=key item=item from=$club_city_list name=categories}{if $item.ct_cn_id>0}
									...
									{else}
										{if $item.ct_cn_id == $club_country_list.0.cn_id}<option value="{$item.ct_id}"{if $stadium_item.std_ct_id == $item.ct_id} selected{/if}>{$item.ct_title_ru}</option>{/if}
									{/if}
								{/if}{/foreach}
								</select>
							</td>
