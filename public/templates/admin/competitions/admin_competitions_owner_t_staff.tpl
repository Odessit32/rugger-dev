		<a name="owner_t" id="owner_t"></a><h1>Основной состав команды: &laquo;<a href="?show=team&get=edit&item={$game_item.g_owner_t_id}">{$game_item.g_owner_t_title_ru}</a>&raquo;</h1>
			<table width="100%" cellspacing="0" cellpadding="0" border="0">
				<tr><td colspan="3" height="1" bgcolor="#b2b2b2"></td></tr>
				<tr>
					<td width="33%" id="{if $app_type == 'player'}active_left{else}notactive{/if}"><a href="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type=player&cont=4">Игроки</a></td>
					<td width="34%" id="{if $app_type == 'head'}active{else}notactive{/if}"><a href="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type=head&cont=4">Руководство</a></td>
					<td width="33%" id="{if $app_type == 'rest'}active_right{else}notactive{/if}"><a href="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type=rest&cont=4">Другие</a></td>
				</tr>
			</table>
		
		<br><br>
		<table width="90%" cellspacing="0" cellpadding="0" border="0" id="form_input">
		<form method="post" action="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type={$app_type}&cont=4" onsubmit="if (!confirm('Вы уверены?')) return false">
		{if $game_item.g_is_done == 'no'}
			<tr>
				<td colspan="5">
				<center>
					<input type="submit" name="save_owner_st" id="submitsave" value="Сохранить">
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<input type="submit" name="clear_owner_st" id="submitdelete" value="Очистить">
				</center>
				<br><br>
				</td>
			</tr>
		{/if}
			<tr>
				<th>№</th>
				<th><a href="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type={$smarty.get.app_type}&sort=fio&cont=4">ФИО</a></th>
				<th>Осн. состав {if $game_item.g_is_done == 'no'}<input type="checkbox" onclick="{foreach key=key item=item from=$game_item.g_owner_t_staff name=staff}this.form.main_{$smarty.foreach.staff.iteration}_{$item.st_id}.checked=this.checked; this.form.reserve_{$smarty.foreach.staff.iteration}_{$item.st_id}.checked=false; {/foreach}" id="input">{/if}</th>
				<th><a href="?show=competitions&ch={$smarty.get.ch}&get=edit&item={$smarty.get.item}&edit_comp=games_edit&g_item={$smarty.get.g_item}&app_type={$smarty.get.app_type}&cont=4">Позиция</a></th>
				<th>Замена</th>
			</tr>
		<input type="hidden" name="t_id" value="{$game_item.g_owner_t_id}">
		<input type="hidden" name="g_id" value="{$game_item.g_id}">
		{if $game_item.g_owner_t_staff}
			{foreach key=key item=item from=$game_item.g_owner_t_staff name=staff}
