		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0">
			...
			<tr>
				<td width="33%" id="{if empty($smarty.get.g_get) || $smarty.get.g_get == 'photos'}active_left{else}notactive{/if}"><a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4">Список</a></td>
				<td width="34%" id="{if !empty($smarty.get.g_get) && $smarty.get.g_get == 'edit'}active{else}notactive{/if}">{if !empty($smarty.get.g_f_item)}<a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4&g_get=edit&g_f_item={$smarty.get.g_f_item}">Редактировать</a>{elseif !empty($smarty.get.g_v_item)}<a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4&g_get=edit&g_v_item={$smarty.get.g_v_item}">Редактировать</a>{else}Редактировать{/if}</td>
				<td width="33%" id="{if !empty($smarty.get.g_get) && $smarty.get.g_get == 'add'}active_right{else}notactive{/if}"><a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4&g_get=add">Добавить</a></td>
			</tr>
		</table>
	{if empty($smarty.get.g_get)}
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0">
			<tr><td>
			{if $staff_photo_list}
			{foreach key=key item=item from=$staff_photo_list name=images}
				<div class="image_item{if $item.ph_type_main == 'yes'}_main{/if}">
					<a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4&g_get=edit&g_f_item={$item.ph_id}"><img src="../upload/photos{$item.ph_folder}{$item.ph_path}?{$smarty.now}" width="166"></a>
		...
		<table width="100%" cellspacing="0" cellpadding="0" border="0">
			<tr><td>
			{if $staff_video_list}
			{foreach key=key item=item from=$staff_video_list name=images}
				<div class="image_item">
					<a href="?show=staff&get=edit&item={$smarty.get.item}&cont=4&g_get=edit&g_v_item={$item.v_id}"><img src="../upload/video_thumbs{$item.v_folder}{$item.v_id}-med.jpg?{$smarty.now}" width="166"></a>
			...
			<table width="95%" cellspacing="0" cellpadding="0" border="0">
				<tr>
					...
					<table width="100%" cellspacing="0" cellpadding="0" border="0" id="form_input">
						...
						<tr>
							...
							<select name="ph_gallery_id" id="input100">
								{if $staff_photo_gallery}<option value="{$staff_photo_gallery.phg_id}" selected>{$staff_photo_gallery.phg_title_ru}</option>{else}<option value="new" selected>Новая: {$staff_item.st_family_ru} {$staff_item.st_name_ru} {$staff_item.st_surname_ru}</option>{/if}
								<option value="0">без галереи</option>
							{if $gallery_list}
							{foreach key=key item=item from=$gallery_list name=images}
								{if $staff_photo_gallery.phg_id != $item.id}<option value="{$item.id}"{if $item.id == $photo_item.ph_gallery_id} selected{/if}>{$item.title}</option>{/if}
							{/foreach}
							{/if}
							</select>
							</td>
                                ...
                                <select name="phg_phc_id" id="input100">
                                    ...
                                    {if $photo_category_list}
                                        {foreach key=key item=item from=$photo_category_list name=category}
                                            <option value="{$item.phc_id}"{if $staff_photo_gallery.phg_phc_id == $item.phc_id} selected="selected"{/if}>{$item.phc_title_ru}</option>
                                        {/foreach}
                                    {/if}
                                </select>
                            </td>
					...
					<td width="50%" align="center" valign="middle">
						<input type="submit" name="add_new_staff_photo" id="submitsave" value="Добавить">
					</td>
				</tr>
			</table>
			<br>
		...
		<form method="post" enctype="multipart/form-data">
			...
			<table width="95%" cellspacing="0" cellpadding="0" border="0">
				<tr>
					...
					<table width="100%" cellspacing="0" cellpadding="0" border="0" id="form_input">
						...
						<tr>
							...
							<select name="v_gallery_id" id="input100">
								{if $staff_video_gallery}<option value="{$staff_video_gallery.vg_id}" selected>{$staff_video_gallery.vg_title_ru}</option>{else}<option value="new" selected>Новая: {$staff_item.st_family_ru} {$staff_item.st_name_ru} {$staff_item.st_surname_ru}</option>{/if}
								<option value="0">без галереи</option>
							{if $gallery_list_video}
							{foreach key=key item=item from=$gallery_list_video name=images}
								{if $staff_video_gallery.vg_id != $item.id}<option value="{$item.id}">{$item.title}</option>{/if}
							{/foreach}
							{/if}
							</select>
							</td>
                                ...
                                <select name="vg_vc_id" id="input100">
                                    ...
                                    {if $video_category_list}
                                        {foreach key=key item=item from=$video_category_list name=category}
                                            <option value="{$item.vc_id}"{if $staff_video_gallery.vg_vc_id == $item.vc_id} selected="selected"{/if}>{$item.vc_title_ru}</option>
                                        {/foreach}
                                    {/if}
                                </select>
                            </td>
					...
					<td width="50%" align="center" valign="middle">
						<input type="submit" name="add_new_staff_video" id="submitsave" value="Добавить">
					</td>
				</tr>
			</table>
			<br>
				...
				<tr>
					...
					<table width="100%" cellspacing="0" cellpadding="0" border="0" id="form_input">
						...
						<tr>
							...
							<td width="50%" align="center">
							<select name="ph_gallery_id" id="input100">
								<option value="{$staff_photo_gallery.id}"{if $staff_photo_gallery.id == $photo_item.ph_gallery_id} selected{/if}>{$staff_photo_gallery.title}</option>
								<option value="0"{if $photo_item.ph_gallery_id == 0} selected{/if}>без галереи</option>
							{if $gallery_list}
							{foreach key=key item=item from=$gallery_list name=images}
								{if $staff_photo_gallery.id != $item.id}<option value="{$item.id}"{if $item.id == $photo_item.ph_gallery_id} selected{/if}>{$item.title}</option>{/if}
							{/foreach}
							{/if}
							</select>
							</td>
                        ...
                        <tr>
                            ...
                            <td>
                                <select name="phg_phc_id" id="input100">
                                    ...
                                    {if $photo_category_list}
                                        {foreach key=key item=item from=$photo_category_list name=category}
                                            <option value="{if $staff_photo_gallery.phg_phc_id == $item.phc_id} selected="selected"{/if}>{$item.phc_title_ru}</option>
                                        {/foreach}
                                    {/if}
                                </select>
                            </td>
					...
					<td width="50%" align="center" valign="middle">
						<input type="submit" name="save_edited_staff_photo" id="submitsave" value="Сохранить">
						<br><br><br>
						<input type="submit" name="delete_edited_staff_photo" id="submitdelete" value="Удалить">
					</td>
				</tr>
			</table>
			<br>
				...
				<tr>
					...
					<table width="100%" cellspacing="0" cellpadding="0" border="0" id="form_input">
						...
						<tr>
							...
							<td width="50%" align="center">
								<select name="v_gallery_id" id="input100">
									{if $staff_video_gallery}<option value="{$staff_video_gallery.vg_id}">{$staff_video_gallery.vg_title_ru}</option>{else}<option value="new" selected>Новая: {$staff_item.st_family_ru} {$staff_item.st_name_ru} {$staff_item.st_surname_ru}</option>{/if}
									<option value="0"{if $video_item.v_gallery_id == 0} selected{/if}>без галереи</option>
								{if $gallery_list_video}
								{foreach key=key item=item from=$gallery_list_video name=images}
									{if $staff_video_gallery.vg_id != $item.id}<option value="{$item.id}"{if $item.id == $video_item.v_gallery_id} selected{/if}>{$item.title}</option>{/if}
								{/foreach}
								{/if}
								</select>
							</td>
					...
					<td width="50%" align="center" valign="middle">
						<input type="submit" name="save_edited_staff_video" id="submitsave" value="Сохранить">
						<br><br><br>
						<input type="submit" name="delete_edited_staff_video" id="submitdelete" value="Удалить">
					</td>
				</tr>
			</table>
			<br>
			...
			<tr>
				...
				<form method="post" enctype="multipart/form-data">
					...
					<tr>
						...
						<br>
						<input type="submit" name="save_edited_staff_preview" id="submitsave" value="Перезаписать">
						</td>
					</tr>
				</form>
				</table>
