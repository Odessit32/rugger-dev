    ...
    <div class="left_menu overflow">
        <ul>
            {foreach key=key item=item from=$news_categories}
                <li><a href="{$sitepath}{$page_item.page_path}{$page_item.p_adress}{if $item.nc_address != ''}/{$item.nc_address}{/if}{if $date_show}/date-{$date_show}/{/if}" title="{$item.title}"{if !empty($item.active) && $item.active == 'yes'} class="active"{/if}>{$item.title}</a></li>
            {/foreach}
