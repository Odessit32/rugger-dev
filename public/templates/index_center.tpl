{strip}
{if $mbi.center}
	{foreach key=key item=item from=$mbi.center name=b_center}
		{if $item.mbi_item_type == 'banner'}
			{if $item.item.b_type == 'image'}
				{if $item.item.b_noindex == 'yes'}<noindex>{/if}
				<a href="{$item.item.url}" title="{$item.item.title}"{if $item.item.target == 'yes'} target="_blank"{/if}><img src="{$imagepath}upload/banners{$item.item.path}" style="max-width: 230px;" border="0" loading="lazy" alt="{$item.item.title}"></a>
				{if $item.item.b_noindex == 'yes'}</noindex>{/if}
