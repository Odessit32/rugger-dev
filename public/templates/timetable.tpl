            ...
            <div class="page_content">
                ...
                {if isset($is_empty) && !$is_empty}
                    <div class="content_post">
                        ...
                        {/if}
                        <div class="results">
                            {if isset($timetable_list.soon) && $timetable_list.soon}
                                ...
                                {foreach item=item_m from=$timetable_list.soon name=soon_m}
                                                ...
                                                <div class="games_list_">
                                                    {foreach item=item from=$item_ch.data name=soon}
                                                        {if isset($item.an_type) && $item.an_type == 'game'}
                                                            {if isset($item.owner) && isset($item.owner.title) && isset($item.guest) && isset($item.guest.title)}
                                                                {if isset($item.is_detailed) && $item.is_detailed}<a href="{$sitepath}{if isset($page_item.page_path)}{$page_item.page_path}{/if}game/{if isset($item.g_id)}{$item.g_id}{/if}" title="{$item.owner.title} - {$item.guest.title}" class="games_list_item game-{$item.g_id|default:''}{if isset($item.g_selected) && $item.g_selected == 'yes'} selected{/if}">{else}<span class="games_list_item game-{$item.g_id|default:''}{if isset($item.g_selected) && $item.g_selected == 'yes'} selected{/if}">{/if}
                                                                    <span class="title_left">{$item.owner.title}</span>
                                                                    <span class="points"> : </span>
                                                                    <span class="title_right">{$item.guest.title}</span>
                                                                    {if isset($item.g_is_schedule_time) && $item.g_is_schedule_time == 'yes' && isset($item.date_gmt) && isset($item.time_zone) && isset($item.datetime_gmt)}
                                                                        <span class="date date_calculate" data-date-gmt="{$item.date_gmt}" data-time-zone="{$item.time_zone}">({$item.datetime_gmt|date_format:"%H:%M"} {$language.gmt|default:'GMT'})</span>
                                                                    {/if}
                                                                    {if isset($item.g_info) && is_array($item.g_info) && !empty($item.g_info.live)}
                                                                        <span class="live">live</span>
                                                                    {/if}
                                                                {if isset($item.is_detailed) && $item.is_detailed}</a>{else}</span>{/if}
                                                            {else}
                                                                <span class="games_list_item game-{$item.g_id|default:''}">Команды не найдены</span>
                                                            {/if}
                                                        {elseif isset($item.an_type) && $item.an_type == 'competition'}
                                                            {if isset($item.chl_address) && isset($item.chg_address) && isset($item.ch_address) && isset($item.cp_tour) && isset($item.cp_substage) && isset($item.g_cp_id) && isset($item.chl_title) && isset($item.chg_title) && isset($item.ch_title) && isset($item.cp_title)}
                                                                <a class="competition_title" href="{$sitepath}competitions/{$item.chl_address}/{$item.chg_address}/{$item.ch_address}/{$item.cp_tour}/{$item.cp_substage}/{$item.g_cp_id}">
                                                                    {$item.chl_title}. {$item.chg_title}. {$item.ch_title}. {$item.cp_title}. ({$language.competition_tour_title|default:'Тур'} {$item.cp_tour+1}/{$item.cp_substage})
                                                                </a>
                                                            {else}
                                                                <span class="competition_title">Данные о соревновании неполные</span>
                                                            {/if}
                                                        {/if}
                                                    {/foreach}
                                                </div>
                ...
                {else}
                    <div class="content_post">
                        <p>{$language.no_timetables|default:'Нет расписания'}</p>
                    </div>
                {/if}
                {if isset($page_item.files) && $page_item.files && isset($page_item.p_is_files) && $page_item.p_is_files == 'yes'}
                    <div class="files">
                        {if isset($page_item.p_title_files) && $page_item.p_title_files != ''}<h3>{$page_item.p_title_files}</h3>{/if}
                        {foreach key=key item=item from=$page_item.files}
                            <div class="item">
                                <div class="text">
                                    {if isset($item.about)}{$item.about}{/if}<br>
                                    <a href="{$imagepath}upload/files{if isset($item.f_folder)}{$item.f_folder}{/if}{if isset($item.f_path)}{$item.f_path}{/if}">{if isset($item.f_path)}{$item.f_path}{/if}</a> <i>({if isset($item.size)}{$item.size}{/if}).</i>
                                </div>
                                <div class="ico">
                                    <a href="{$imagepath}upload/files{if isset($item.f_folder)}{$item.f_folder}{/if}{if isset($item.f_path)}{$item.f_path}{/if}"><img src="{$imagepath}{if isset($item.ico)}{$item.ico}{/if}" border="0"></a>
                                </div>
                            </div>
                        {/foreach}
                    </div>
                {/if}
                {if isset($page_item.pvs) && $page_item.pvs}
                    <div class="pvs">
                        {foreach key=key item=item from=$page_item.pvs}
                            {if isset($item.pvs_service) && $item.pvs_service == 'youtube'}<iframe width="460" height="400" src="//www.youtube.com/embed/{if isset($item.pvs_code)}{$item.pvs_code}{/if}" frameborder="0" allowfullscreen></iframe>{/if}
                        {/foreach}
                    </div>
                {/if}
            </div>
...
<script>
    $(window).on('load', function(){
        ...
        var dates = [{/literal}{if isset($timetable_list.soon_date_list) && $timetable_list.soon_date_list}{$timetable_list.soon_date_list}{/if}{literal}];
        if ($.fn.datepicker) {
